...
enum {
  INFO_IMU = 0,    // Set 1 to info IMU states
  INFO_MOTOR = 0,  // Set 1 to info motor states
  HIGH_FREQ =
 ...
 public:
  low_level_cmd_sender() : Node("low_level_cmd_sender") {
    ...
    lowstate_subscriber_ = this->create_subscription<unitree_hg::msg::LowState>(
        topic_name, 10,
        [this](const std::shared_ptr<unitree_hg::msg::LowState> message) { LowStateHandler(message); });
 ...
 private:
  void Control() {
    ...
    if (time_ < duration_) {
      ...
      for (int i = 0; i < H1_2_NUM_MOTOR; ++i) {
        double const ratio =
            unitree::common::clamp(time_ / duration_, 0.0, 1.0);
