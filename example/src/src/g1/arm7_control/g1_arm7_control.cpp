 ...
 public:
  g1_arm7_control_sender() : Node("g1_arm7_control_sender") {
    ...
    mLowstateSubscriber = this->create_subscription<unitree_hg::msg::LowState>(
        topic_name, 10,
        std::bind(&g1_arm7_control_sender::LowStateHandler, this, _1));
 ...
 private:
  ...
  void InitArms() {
    ...
    for (size_t i = 0; i < mLowCmdParam.arm_joints.size(); ++i) {
      current_jpos.at(i) =
          mStateMsg->motor_state.at(mLowCmdParam.arm_joints.at(i)).q;
    ...
    int init_time_steps =
        static_cast<int>(mLowCmdParam.init_time / mLowCmdParam.control_dt);
    for (int i = 0; i < init_time_steps; ++i) {
      ...
      for (size_t j = 0; j < mLowCmdParam.init_pos.size(); ++j) {
        mCmd.motor_cmd.at(mLowCmdParam.arm_joints.at(j))
            .set__q(mLowCmdParam.init_pos.at(j) * phase +
                    current_jpos.at(j) * (1 - phase));
        mCmd.motor_cmd.at(mLowCmdParam.arm_joints.at(j))
            .set__dq(mLowCmdParam.dq);
        mCmd.motor_cmd.at(mLowCmdParam.arm_joints.at(j))
            .set__kp(mLowCmdParam.kp);
        mCmd.motor_cmd.at(mLowCmdParam.arm_joints.at(j))
            .set__kd(mLowCmdParam.kd);
        mCmd.motor_cmd.at(mLowCmdParam.arm_joints.at(j))
            .set__tau(mLowCmdParam.tau_ff);
      }
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(mLowCmdParam.sleep_time));
    }
  ...
  void LiftArmsUp() {
    ...
    for (int i = 0; i < mLowCmdParam.num_time_steps; ++i) {
      // update jpos des
      for (size_t j = 0; j < mLowCmdParam.init_pos.size(); ++j) {
        mLowCmdParam.current_jpos_des.at(j) += unitree::common::clamp(
            mLowCmdParam.target_pos.at(j) - mLowCmdParam.current_jpos_des.at(j),
      ...
      for (size_t j = 0; j < mLowCmdParam.init_pos.size(); ++j) {
        mCmd.motor_cmd.at(mLowCmdParam.arm_joints.at(j))
            .set__q(mLowCmdParam.current_jpos_des.at(j));
        mCmd.motor_cmd.at(mLowCmdParam.arm_joints.at(j))
            .set__dq(mLowCmdParam.dq);
        mCmd.motor_cmd.at(mLowCmdParam.arm_joints.at(j))
            .set__kp(mLowCmdParam.kp);
  ...
  void PutArmsDown() {
    ...
    for (int32_t i = 0; i < mLowCmdParam.num_time_steps; ++i) {
      ...
      for (size_t j = 0; j < mLowCmdParam.init_pos.size(); ++j) {
        mLowCmdParam.current_jpos_des.at(j) += unitree::common::clamp(
            mLowCmdParam.init_pos.at(j) - mLowCmdParam.current_jpos_des.at(j),
            -mLowCmdParam.max_joint_delta, mLowCmdParam.max_joint_delta);
      }
  ...
  void StopControl() {
    ...
    for (int i = 0; i < stop_time_steps; ++i) {
      ...
      mLowCmdParam.weight =
          unitree::common::clamp(mLowCmdParam.weight, 0.f, 1.f);
    ...
    if (mStateMsg == nullptr) {
      ...
      if (mStateMsg == nullptr) {
        ...
        mTimer = this->create_wall_timer(
            ...
            std::bind(&g1_arm7_control_sender::Control, this));
      }
    }
  }
