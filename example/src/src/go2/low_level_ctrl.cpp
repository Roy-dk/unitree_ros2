...
class low_level_cmd_sender : public rclcpp::Node {
 ...
 private:
  void timer_callback() {
    ...
    cmd_msg_.motor_cmd[RL_2].q = POS_STOP_F;  // Set to stop position(rad)
    cmd_msg_.motor_cmd[RL_2].kp = 0;
    cmd_msg_.motor_cmd[RL_2].dq =
        VEL_STOP_F;  // Set to stop angular velocity(rad/s)
  ...
  void init_cmd() {
    for (int i = 0; i < 20; i++) {
      ...
      cmd_msg_.motor_cmd[i].q = POS_STOP_F;
      cmd_msg_.motor_cmd[i].kp = 0;
      cmd_msg_.motor_cmd[i].dq = VEL_STOP_F;
