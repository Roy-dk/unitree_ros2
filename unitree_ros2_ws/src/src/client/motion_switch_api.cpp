...
int32_t MotionSwitchApi::SelectModeRes(
    const std::shared_ptr<unitree_api::srv::Generic::Response> &res) {
  return res->code;
}

void MotionSwitchApi::ReleaseModeReq(
    const std::shared_ptr<unitree_api::srv::Generic::Request> &req) {
  req->api_id = MOTION_SWITCHER_API_ID_RELEASE_MODE;
}

int32_t MotionSwitchApi::ReleaseModeRes(
    const std::shared_ptr<unitree_api::srv::Generic::Response> &res) {
  return res->code;
}

void MotionSwitchApi::SetSilentReq(
    bool silent,
    const std::shared_ptr<unitree_api::srv::Generic::Request> &req) {
  nlohmann::json js;
  js["silent"] = silent;
...
int32_t MotionSwitchApi::SetSilentRes(
    const std::shared_ptr<unitree_api::srv::Generic::Response> &res) {
  return res->code;
}

void MotionSwitchApi::GetSilentReq(
    const std::shared_ptr<unitree_api::srv::Generic::Request> &req) {
  req->api_id = MOTION_SWITCHER_API_ID_GET_SILENT;
}

int32_t MotionSwitchApi::GetSilentRes(
    const std::shared_ptr<unitree_api::srv::Generic::Response> &res,
    bool &silent) {
  if (res->code == 0) {
    auto js = nlohmann::json::parse(res->data.c_str());
