...
LocoClient::LocoClient(const std::string& nodeName)
    : BaseClient(nodeName, LOCO_SERVICE_NAME) {}
...
int32_t LocoClient::SetFsmId(int32_t fsmId){
    SEND_LOCO_REQUEST(SetFsmIdReq, fsmId) PARSE_LOCO_RESPONSE(SetFsmIdRes)}
...
int32_t LocoClient::SetTaskId(int32_t taskId){
    SEND_LOCO_REQUEST(SetTaskIdReq, taskId) PARSE_LOCO_RESPONSE(SetTaskIdRes)}
