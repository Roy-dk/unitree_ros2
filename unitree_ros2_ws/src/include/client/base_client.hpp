...
#define SEND_REQUEST(PARAM, REQUEST_FUNC, ...)                           \
  ...
  if (ret != 0) {                                                        \
    RCLCPP_ERROR(this->get_logger(), "%s api call failed . ret : %d",    \
                 __FUNCTION__, ret);                                     \
  }                                                                      \
  if (response == nullptr) {                                             \
    return ret;                                                          \
...
class BaseClient : public rclcpp::Node, public ClientConfig {
 ...
 protected:
  int32_t Call(
      const std::shared_ptr<unitree_api::srv::Generic::Request> &request,
      std::shared_ptr<unitree_api::srv::Generic::Response> &response) {
    if (!wait_service()) {
      RCLCPP_ERROR(this->get_logger(), "service %s is not ready",
                   mServiceName.c_str());
  ...
  bool wait_service() {
    ...
    while (!mClient->wait_for_service(
      ...
      if (!rclcpp::ok()) {
        RCLCPP_ERROR(
            this->get_logger(),
            "Interrupted while waiting for the service %s to be available...",
            mServiceName.c_str());
        return false;
      }
      ++tryTimes;
      if (tryTimes >= WAIT_ROS_SERVICE_MAX_TRY_TIME) {
        RCLCPP_ERROR(this->get_logger(), "Failed to find service : %s",
                     mServiceName.c_str());
        return false;
      }
      RCLCPP_INFO(this->get_logger(),
                  "Waiting for the service %s to be available...",
                  mServiceName.c_str());
    }
